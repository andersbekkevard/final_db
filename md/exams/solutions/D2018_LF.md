...
ER-diagram (samlet modell over fire bilder):

**Entitetsklasser:**
- Lag (LagID [PK], Navn, StiftetÅr)
- Arena (Navn [PK], Kapasitet)
- Kamp (KampNr [PK], Dato, StartTid, Serierunde, Tilskuere, Kamplengde)
- Person (PID [PK], Navn, FÅr, Mobil) — supertype med disjunkte (totale) subtyper:
  - Spiller (TroyeNr, Type)
  - Trener
  - Dommer (Status)

**Relasjonsklasser:**
- Hjemmebane: Lag (1,1) — (1,2) Arena
- SpillesPå: Kamp (1,1) — (0,n) Arena
- HjemmeLag: Lag (0,n) — (1,1) Kamp [Mål]
- BorteLag: Lag (0,n) — (1,1) Kamp [Mål]
- SpillerKamp: Spiller (0,n) — (0,n) Kamp [FraTid, TilTid, Mål, Selvmål, Gult, Rødt, Karakter]
- DommerKamp: Dommer (0,n) — (4,4) Kamp [Rolle]
- SpillerFor: Spiller (1,1) — (0,n) Lag
- TrenerFor: Trener (1,1) — (0,n) Lag [Rolle]
- Medlem: Dommer (0,1) — (0,n) Lag [Status]
...
**Entitetsklasser:**
- Genre (ID [PK], Title, Description)
- Book (ID [PK], Title, PublYear, Price)
- Publisher (ID [PK], Name, URL)
- Author (ID [PK], Firstname, Surname, Nationality, URL)

**Relasjonsklasser:**
- BookGenre: Genre (0,n) — (0,n) Book
- PublishedBy: Book (0,1) — (0,n) Publisher
- BookAuthor: Book (0,n) — (0,n) Author
...
Tre uttrykk i et tre. Venstre delre: π_{BookID}( (σ_{Firstname='Linn' ∧ Surname='Ullmann'}(Author)) ⋈_{ID=AuthorID} (BookAuthor AS BA1) ). Høyre delre: (σ_{Firstname≠'Linn' ∨ Surname≠'Ullmann'}(Author)) ⋈_{ID=AuthorID} (BookAuthor AS BA2). De to delrene kobles sammen med en join på BA1.BookID = BA2.BookID, og resultatet projiseres på {ID, Firstname, Surname, Nationality}.

```
π_{ID, Firstname, Surname, Nationality}
  └─ ⋈_{BA1.BookID = BA2.BookID}
      ├─ π_{BookID}
      │   └─ ⋈_{ID = AuthorID}
      │       ├─ σ_{Firstname='Linn' ∧ Surname='Ullmann'} (Author)
      │       └─ ρ_{BA1} (BookAuthor)
      └─ ⋈_{ID = AuthorID}
          ├─ σ_{Firstname≠'Linn' ∨ Surname≠'Ullmann'} (Author)
          └─ ρ_{BA2} (BookAuthor)
...
**Strukturell beskrivelse av diagrammet (med markerte feil):**

- Entitetsklasse Course (CourseID, CourseName) — feil 1: CourseID er ikke understreket som nøkkel.
- Entitetsklasse Student (SID [PK], Name).
- Relasjonsklasse Exam mellom Course og Student med attributtene Grade og Date — kardinalitet (0,n) på begge sider. Feil 4: en student kan kun ha én eksamen i samme emne slik det er modellert (burde tillate flere).
- Relasjonsklasse Registration mellom Course og Student med attributt Date — feil 2: kardinaliteten (1,n) på Course-siden tilsier at alle emner må ha minst én student; dette burde vært (0,n).
- Svak entitetsklasse ExercisePackage (ExID, Year) tegnet med dobbel strek — feil 3: ExercisePackage tilfredsstiller ikke kravene til å være svak entitetsklasse.
- Tertiær relasjonsklasse Completed (med attributt Status) mellom Course, Student og ExercisePackage — feil 5: burde vært to binære relasjonsklasser i stedet for én tertiær.
...
PDF viser fire mellomliggende tilstander (en rett før hver splitt) pluss endelig treetilstand. Hver tilstand er tatt før neste innsetting forårsaker en splitt.

**Tilstand 1 — full løvblokk, rett før splitt utløst av nøkkel 17:**

```
root -> [3, 4, 28]
```

**Tilstand 2 — rett før splitt utløst av nøkkel 18:**

(17 er allerede satt inn: [3,4,28] ble splittet til [3,4] og [28], 28 ble promotert, og 17 ble plassert i venstre løv som da ble [3,4,17]. Nøkkel 18 skal nå inn i full venstre løv.)
...
**Tilstand 3 — rett før splitt utløst av nøkkel 13:**

(Etter splitt for 18 og innsetting av 5, 27.)
...
**Tilstand 4 — rett før splitt utløst av nøkkel 15:**

(Etter splitt for 13 og innsetting av 16.)

```
            root -> [5, 17, 28]
                   /    |     |     \
       [3, 4] <-> [5, 13, 16] <-> [17, 18, 27] <-> [28]
```
...
**Endelig tilstand** (etter at 15 utløser leaf-splitt og påfølgende rotsplitt; treet får tre nivå):

[5, 13, 16] splittes til [5, 13] og [16], 16 promoteres → root blir [5, 16, 17, 28] (4 nøkler, for stor). Indre splitt: [5, 16, 17, 28] deles i [5, 16] og [28]; 17 promoteres til ny rot.
...
Hvor mange blokker aksesseres ved utføring av de følgende queries. Begrunn svarene dine.
...
Her er ikke B+-treet til noe særlig hjelp. Studentens øvinger er spredd utover løvnivået i treet. Dvs. vi må gå ned treet og scanne løvnivået. `2 + 1000 = 1002` blokker. Noen studenter har ikke tegnet inn sidepekere i B+-treet i oppgave 5, likevel har de på magisk vis greid å scanne løvnivået i oppgave b)-d) her.
