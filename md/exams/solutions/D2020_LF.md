...
- `Paper` med attributter: Title, ID (PK), PDF, Type, Status
- `Author` med attributter: ID (PK), Name, Email
- `Review` med attributter: No (PK), Reviewer-Name, Grade, Rewrite, OpenComment, SecretComment
- `ProgComm-Member` med attributter: ID (PK), Name, Affiliation, Nationality
- `Category` med attributter: Title (PK), Description

**Relasjoner:**

- `ReviewOf` (ternær mellom Paper, Author og Review):
  - Paper-siden: (0,n)
  - Author-siden: (0,n)
  - Review-siden: (1,n)
- `Responsible` (binær mellom Review og ProgComm-Member):
  - Review-siden: (1,1)
  - ProgComm-Member-siden: (0,n)
  - Attributt på relasjonen: Competence
- `ReviewCategory` (binær mellom Review og Category):
  - Review-siden: (1,1)
  - Category-siden: (0,n)
...
- `A` med attributter: a-1 (PK), a-2 (sammensatt med delkomponentene a-3 og a-4)
- `B` med attributter: b-1 (PK), b-2 (flerverdi-attributt)
- `C` (sub-klasse av B)
- `D` (sub-klasse av B)
- `E` med attributter: e-1 (PK), e-2, e-3

**ISA/spesialisering:**

- B har en spesialisering med disjunkte sub-klasser C og D (delvis dekning – ikke total).

**Relasjoner:**

- `R` (binær mellom A og B):
  - A-siden: (0,n)
  - B-siden: (0,1)
- `S` (binær mellom D og E):
  - D-siden: (0,n)
  - E-siden: (0,n)
  - Attributt på relasjonen: s-1
...
Trestrukturen for spørringen (tegnet ovenfra og ned, med boksene A-F som ukjente):
...
Med andre ord: vi starter med å selektere County med en betingelse (A), joiner (boks C) med en annen relasjon (B), kombinerer dette via en mellomoperator (D – tilsynelatende en naturlig join eller `*`) med resultatet av seleksjonen `Number > 0` på Infected, før vi joiner (E) med RegisteredDates, projiserer DD, MM, YY og til slutt sorterer (F).
...
Det skisserte B+-treet (allerede delvis utfylt) har følgende struktur:
...
Rotnoden inneholder separator-nøkkelen 15. Venstre interne node inneholder separator-nøkkelen 6, mens høyre interne node inneholder separator-nøklene 23 og 28. Løvnodene (a)-(e) er lenket sammen som søsken via `→`-pekere.
